...
zz.Internal.API.Files.Attachment.get = function(aspect, successCallback, errorCallback) {
    ...
    if (null != storedData) {
        ...
        if (null != blob && null != successCallback) {
    ...
    zzCloud.ZZ.Internal.Cloud.Files.Attachment.download({
        token: user.token
    }, {
        id: aspect.id
    }, _downloadSuccessCallback, _downloadErrorCallback);
};
...
zz.Internal.API.Files.Attachment.set = function(aspect, content, successCallback, errorCallback) {
    ...
    var path = user.id + "/tmp-attachment-" + user.token + "-" + new Date().getTime();
