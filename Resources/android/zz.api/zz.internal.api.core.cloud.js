...
var _lockPost = function(user, post, successCallback, errorCallback) {
    ...
    storedPost.id = null != storedDataPost.remote_data_ref ? storedDataPost.remote_data_ref : storedDataPost.id;
...
var _uploadAttachments = function(user, post, successCallback, errorCallback) {
    ...
    var storedDataAspects = zzLocalDB.ZZ.Internal.Local.DB.Datas.findByUserRefAndLocalDataRef(user.id, post.id);
    if (null == storedDataAspects || 0 == storedDataAspects.length) {
    ...
    _.each(storedDataAspects, function(storedDataAspect) {
        var _syncAttachmentSuccessCallback = function(response) {
            ...
            calls == storedDataAspects.length && null != successCallback && successCallback();
        };
        ...
        _uploadAttachment(user, {
            id: storedDataAspect.id
        }, _syncAttachmentSuccessCallback, _syncAttachmentErrorCallback);
    });
};
...
var _uploadAttachment = function(user, aspect, successCallback, errorCallback) {
    ...
    if (null == user || null == user.id || null == user.token) {
    ...
    zzCloud.ZZ.Internal.Cloud.Files.Attachment.upload({
        token: user.token
    }, blob, _uploadSuccessCallback, _uploadErrorCallback);
};
...
var _preparePost = function(user, post, successCallback, errorCallback) {
    ...
    var storedDataAspects = zzLocalDB.ZZ.Internal.Local.DB.Datas.findByUserRefAndLocalDataRef(user.id, post.id);
    if ((null == storedDataAspects || 0 == storedDataAspects.length) && storedDataPost.status == zzLocalDB.ZZ.Internal.Local.DB.Data.CONSTANTS.STATUSES.SYNC) {
    ...
    storedPost.aspects = [];
    if (null != storedDataAspects && storedDataAspects.length > 0) {
        _.each(storedDataAspects, function(storedDataAspect) {
            if (storedDataAspect.status == zzLocalDB.ZZ.Internal.Local.DB.Data.CONSTANTS.STATUSES.DELETED) return;
            var storedAspect = JSON.parse(storedDataAspect.serialized_data);
...
var _uploadPost = function(user, post, successCallback, errorCallback) {
    ...
    } else storedDataPost.status == zzLocalDB.ZZ.Internal.Local.DB.Data.CONSTANTS.STATUSES.UPDATED || Ti.API.debug("ZZ.Sync.syncPost._sync unable to perform upload due to NOT managed status : " + storedData.status);
};
...
var _syncPost = function(user, localPost, remotePost, successCallback, errorCallback) {
    ...
    if (null == localPost || null == localPost.id) {
    ...
    if (null == remotePost || null == remotePost.id) {
    ...
    if (null != remotePost.aspects && remotePost.aspects.length > 0) {
        var canContinue = _.every(remotePost.aspects, function(aspect) {
            if (null != aspect.data && null != aspect.data.content && null != aspect.data.content.local) {
                ...
                var path = user.id + "/attachments-" + aspect.id;
                ...
                var done = zzLocalFS.ZZ.Internal.Local.FS.File.delete(storedDataFile.local_fs_ref);
                if (!done) {
    ...
    null != successCallback && successCallback(remotePost);
};
...
var _syncPosts = function(user, post, successCallback, errorCallback) {
    ...
    null != successCallback && successCallback(posts);
};