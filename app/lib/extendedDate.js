exports.extentedData = function() {
	...
	Date.prototype.addWeekDays = function(d) {
		...
		var wkEnd = 0;
		//number of weekends needed
		var m = d % 5;
		//number of weekdays for partial week

		if (d < 0) {
			wkEnd = Math.ceil(d / 5);
			//Yields a negative number of weekends

			switch (startDay) {
				case 6:
					//Staring day Sat. 1 less weekend
					if (m == 0 && wkEnd < 0)
						wkEnd++;
					break;
				case 0:
					if (m == 0)
						d++;
					//decrease - part of weekend
					else
						d--;
					//increase - part of weekend
					break;
				default:
					if (m <= -startDay)
						wkEnd--;
				//add weekend if not enough days to cover
			}
		} else if (d > 0) {
		...
		d += wkEnd * 2;
		//Add weekends to weekdays needed
